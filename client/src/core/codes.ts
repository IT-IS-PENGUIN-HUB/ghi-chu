 ...
 *
 * `WORK_01` — position in today's list, computed from order at render time.
 ...
 */
import { DAILY_PREFIX, type Category, type Project, type Task } from "./model";
...
/** Formats today's positional code, e.g. index 0 in WRK -> "WORK_01". */
export function formatDailyCode(category: Category, index: number): string {
  return `${DAILY_PREFIX[category]}_${String(index + 1).padStart(2, "0")}`;
}
...
export interface DailyEntry {
  task: Task;
  /** "WORK_01" — position in today's list for this category. */
  daily: string;
}
...
export function buildDailyList(
  ...
  const comparators: Record<SortMode, (a: Task, b: Task) => number> = {
    ...
    project: (a, b) =>
      a.project.localeCompare(b.project) || a.created.localeCompare(b.created),
  };
