...
describe("task mutations", () => {
  ...
  it("adds a task and advances only that project's counter", () => {
    ...
    const etcBefore = store
      .getSnapshot()
      .projects.find(p => p.code === "ETC")!.next;
    ...
    expect(store.getSnapshot().projects.find(p => p.code === "ALP")!.next).toBe(
      before.next + 1
    );
    // Counters are per project — adding to ALP must not move ETC's.
    expect(store.getSnapshot().projects.find(p => p.code === "ETC")!.next).toBe(
      etcBefore
    );
  });
  ...
  it("does not reissue an id after a delete", () => {
    ...
    expect(
      store.getSnapshot().tasks.find(t => t.id === first.id)
    ).toBeUndefined();
  });

  it("lifts inline #tags out of the quick-add box", () => {
    const task = store.addTask({
      title: "Gọi khách #dukan #gấp",
      project: "ALP",
    })!;
  ...
  it("does not duplicate a tag across repeated saves", () => {
    ...
    expect(store.getSnapshot().tasks.find(t => t.id === task.id)!.tags).toEqual(
      ["gấp"]
    );
  });
  ...
  it("keeps the permanent id when a task moves project", () => {
    ...
    expect(
      store.getSnapshot().tasks.filter(t => t.id === task.id)
    ).toHaveLength(1);
  });
...
describe("day notes", () => {
  ...
  it("creates no file for a day with no note", () => {
    expect(store.allFiles().some(f => f.path.startsWith("data/days/"))).toBe(
      false
    );
  });
  ...
  it("creates the file only once something is written", () => {
    ...
    expect(
      store.allFiles().some(f => f.path === "data/days/2026/2026-08-17.md")
    ).toBe(true);
...
describe("file format survives a full mutation cycle", () => {
  it("stays parseable after add, toggle, edit and delete", () => {
    ...
    const content = store
      .allFiles()
      .find(f => f.path === paths.project("ALP"))!.content;
...
describe("delete with undo", () => {
  it("restores a deleted task exactly as it was", () => {
    ...
    expect(
      store.getSnapshot().tasks.find(t => t.id === task.id)
    ).toBeUndefined();

    store.restoreTask(before);
    expect(store.getSnapshot().tasks.find(t => t.id === task.id)).toEqual(
      before
    );
  });
  ...
  it("keeps the counter ahead so the restored id never collides", () => {
    ...
    expect(store.getSnapshot().tasks.filter(t => t.id === a.id)).toHaveLength(
      1
    );
  });
});
...
describe("deleting a project", () => {
  ...
  it("removes an empty project and its file", () => {
    ...
    expect(store.getSnapshot().projects.some(p => p.code === "NHAM")).toBe(
      true
    );
...
/**
 * A deleted project used to come back on the next sync: the delete removes a
 * row from projects.md *and* the project's own file, and a merge that handed
 * back either half on its own was enough to rebuild the whole project.
 */
describe("a deleted project stays deleted", () => {
  it("ignores a project file that comes back with no row and no tasks", () => {
    const store = seeded();
    store.createProject("Trùng tên", "PX4", "WRK");
    const file = store
      .allFiles()
      .find(f => f.path === paths.project("PX4"))!.content;
    store.deleteProject("PX4");

    // The remote still had data/tasks/PX4.md, so a pull writes it back.
    store.applyRemote([
      { path: paths.project("PX4"), content: file, sha: "sha-px4" },
    ]);

    expect(store.getSnapshot().projects.some(p => p.code === "PX4")).toBe(
      false
    );
  });

  it("ignores a registry row that comes back with no file", () => {
    const store = seeded();
    store.createProject("Trùng tên", "PX4", "WRK");
    const withRow = store
      .allFiles()
      .find(f => f.path === paths.projects)!.content;
    store.deleteProject("PX4");

    // A three-way merge against a device that never saw the delete unions the
    // rows, so the PX4 row reappears in projects.md on its own.
    store.applyMerged([
      { path: paths.projects, content: withRow, sha: "sha-reg", base: withRow },
    ]);
  ...
  it("still shows a project whose row was lost but whose file holds tasks", () => {
    const store = seeded();
    const rows = store.allFiles().find(f => f.path === paths.projects)!.content;
    const withoutAlp = rows
      .split("\n")
      .filter(line => !line.startsWith("| ALP "))
      .join("\n");

    store.applyRemote([
      { path: paths.projects, content: withoutAlp, sha: "sha-reg" },
    ]);

    expect(store.getSnapshot().projects.some(p => p.code === "ALP")).toBe(true);
    expect(store.getSnapshot().tasks.some(t => t.project === "ALP")).toBe(true);
  });

  it("clears the tombstone once the remote no longer has the file", () => {
    const store = seeded();
    store.createProject("Tạo nhầm", "NHAM", "WRK");
    // Pretend it had been pushed once, so deleting leaves a tombstone.
    store.applyRemote([
      {
        path: paths.project("NHAM"),
        content: store.allFiles().find(f => f.path === paths.project("NHAM"))!
          .content,
        sha: "sha-nham",
      },
    ]);
    store.deleteProject("NHAM");
    expect(
      store.allFiles().some(f => f.path === paths.project("NHAM") && f.deleted)
    ).toBe(true);
    ...
    const remote = new Set(
      store
        .allFiles()
        .map(f => f.path)
        .filter(p => p !== paths.project("NHAM"))
    );
    store.dropMissing(remote);

    // Nothing left to push: a delete queued against a file GitHub no longer
    // has is refused, and the same push would fail on it forever.
    expect(store.allFiles().some(f => f.path === paths.project("NHAM"))).toBe(
      false
    );
  });

  it("refuses to delete the catch-all project", () => {
    const store = seeded();
    for (const task of store
      .getSnapshot()
      .tasks.filter(t => t.project === "ETC")) {
      store.deleteTask(task.id);
    }

    expect(store.deleteProject("ETC").ok).toBe(false);
    expect(store.getSnapshot().projects.some(p => p.code === "ETC")).toBe(true);
  });
});
